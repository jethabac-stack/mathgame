...
function updateHostSetupDisplay() {
  ...
  document.querySelector('.player-item.host .player-name').textContent = state.playerName + ' (Host)';
}
...
function handleMessage(msg) {
  switch (msg.type) {
    ...
    case 'playerJoined':
      if (state.isHost) {
        // Filter out the host from the players list (host is already displayed separately)
        state.players = msg.players.filter(p => p.id !== state.playerId);
        updatePlayersList();
      }
      break;
    case 'gameStart':
...
function updatePlayersList() {
  ...
  list.innerHTML = html;
  document.getElementById('startBtn').disabled = false; // Always allow starting
}

function startMultiplayerGame() {
  if (state.players.length === 0 && !confirm('No other players have joined yet. Start game with just yourself?')) return;
